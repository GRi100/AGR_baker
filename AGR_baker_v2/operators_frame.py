...
def frame_overlay_items(self, context):
    """EnumProperty callback — must keep a global reference to avoid
    Blender's known garbage-collection issue with dynamic enum items."""
    global _frame_overlay_items_cache
    items = [(NONE_OVERLAY_ID, "None", "No overlay, just create border")]
    for fname in list_frame_overlays():
        display = Path(fname).stem
        items.append((fname, display, f"Apply overlay: {display}"))
    _frame_overlay_items_cache = items
    return items
...
class AGR_OT_CreateFrameOnSets(Operator):
    ...
    def execute(self, context):
        ...
        overlay_path = resolve_overlay_path(self.frame_overlay)
        if self.frame_overlay != NONE_OVERLAY_ID and overlay_path is None:
            self.report({'ERROR'}, f"Overlay image not found: {self.frame_overlay}")
            return {'CANCELLED'}
    ...
    def process_texture(self, img, tex_type, overlay_path, scale_factor, material_name, output_folder):
        ...
        if tex_type == 'DiffuseOpacity' and overlay_path:
            try:
                # Force RGBA so palette/grayscale overlays still work
                ...
                # Normal blend mode: overlay is simply placed on top
                result_img = Image.alpha_composite(
                    result_img.convert('RGBA'), overlay_img
                )
    ...
    def draw(self, context):
        ...
        manage_row.operator("agr.add_frame_overlay", text="Import...", icon='IMPORT')
        del_op = manage_row.operator("agr.delete_frame_overlay", text="Delete", icon='TRASH')
        del_op.frame_name = self.frame_overlay
...
class AGR_OT_CreateFrameOnFiles(Operator):
    """Scale down texture files and edge-bleed margins (no overlay).

    For applying a decorative overlay onto files, use
    agr.apply_overlay_to_files instead.
    """
...
class AGR_OT_ApplyOverlayToFiles(Operator):
    """Composite a frame overlay PNG onto selected texture files.

    Unlike create_frame_on_files, this operator does not scale or bleed —
    it just puts the overlay on top of each file. Intended for already
    sized textures that just need a decorative frame.
    """
    ...
    frame_overlay: EnumProperty(
        name="Frame Overlay",
        description="Overlay PNG to composite on top of each file",
        items=frame_overlay_items,
    )
    ...
    def execute(self, context):
        ...
        if self.frame_overlay == NONE_OVERLAY_ID:
            self.report({'ERROR'}, "Select an overlay (not 'None')")
            return {'CANCELLED'}
        ...
        # Always force RGBA so palette/grayscale PNGs composite correctly
        try:
            overlay_src = Image.open(overlay_path).convert('RGBA')
        except Exception as e:
            self.report({'ERROR'}, f"Cannot load overlay: {e}")
            return {'CANCELLED'}
        ...
        for file_elem in self.files:
            ...
            try:
                with Image.open(filepath) as src_img:
                    original_mode = src_img.mode
                    has_alpha = original_mode in ('RGBA', 'LA', 'PA')
                    base = src_img.convert('RGBA')

                size = base.size
                overlay = overlay_src if overlay_src.size == size else overlay_src.resize(size, Image.LANCZOS)
                ...
                print(f"✅ Overlay applied to {file_elem.name}")
                processed += 1

                if result is not composited:
                    result.close()
                composited.close()
                base.close()
                if overlay is not overlay_src:
                    overlay.close()

            except Exception as e:
                print(f"❌ Error applying overlay to {file_elem.name}: {e}")
                errors += 1

        overlay_src.close()

        if errors > 0:
            self.report({'WARNING'}, f"Applied overlay to {processed} files, {errors} errors")
        else:
            self.report({'INFO'}, f"Applied overlay to {processed} files")
        return {'FINISHED'}
...
class AGR_OT_AddFrameOverlay(Operator):
    """Import one or more PNG files as frame overlays"""
    bl_idname = "agr.add_frame_overlay"
    bl_label = "Import Frame Overlay"
    bl_options = {'REGISTER'}
    ...
    def execute(self, context):
        ...
        for f in self.files:
            src = Path(self.directory) / f.name
            ...
            if dst.exists():
                ...
                i = 1
                while dst.exists():
                    dst = target_dir / f"{stem}_{i}{ext}"
                    i += 1

            try:
                shutil.copy2(src, dst)
                copied += 1
                print(f"📥 Imported frame overlay: {dst.name}")
            except Exception as e:
                print(f"❌ Failed to import {src.name}: {e}")

        if copied == 0:
            self.report({'WARNING'}, "No PNG files imported")
            return {'CANCELLED'}

        self.report({'INFO'}, f"Imported {copied} frame overlay(s) to {target_dir}")
        return {'FINISHED'}
...
class AGR_OT_DeleteFrameOverlay(Operator):
    """Delete a frame overlay from the user overlays directory"""
    bl_idname = "agr.delete_frame_overlay"
    bl_label = "Delete Frame Overlay"
    bl_options = {'REGISTER'}

    frame_name: StringProperty(
        name="Frame Name",
        description="Filename of the overlay to delete",
        default=""
    )
    ...
    def execute(self, context):
        ...
        if not target.exists():
            self.report({'ERROR'}, f"File not found: {self.frame_name}")
            return {'CANCELLED'}

        try:
            target.unlink()
            print(f"🗑️ Deleted frame overlay: {self.frame_name}")
            self.report({'INFO'}, f"Deleted overlay: {self.frame_name}")
            return {'FINISHED'}
        except Exception as e:
...
classes = (
    ...
    AGR_OT_ApplyOverlayToFiles,
    AGR_OT_AddFrameOverlay,
    AGR_OT_DeleteFrameOverlay,
)
